...
const {
  ...
  reversedDummyTxId,
  ScriptHelper,
} = require("../src/lib/sensible_nft/ScriptHelper");
const inputSatoshis = 100000;
var tx = ScriptHelper.newTx();
...
describe("Test sCrypt contract UTXO Token In Javascript", () => {
  ...
  before(() => {
    const Token = buildContractClass(
      ScriptHelper.compileContract("token.scrypt")
    );
    token = new Token();
  ...
  it("should succeed when two tokens are merged", () => {
    ...
    const dataPart0 =
      toHex(publicKey1) + num2bin(x0, DataLen) + num2bin(x1, DataLen);
    ...
    const dataPart1 =
      toHex(publicKey2) + num2bin(y0, DataLen) + num2bin(y1, DataLen);
    ...
    const testMerge = (inputIndex, balance0, balance1) => {
      ...
      const prevouts =
        reversedDummyTxId + num2bin(0, 4) + reversedDummyTxId + num2bin(1, 4);

      const newLockingScript0 = [
        lockingScriptCodePart,
        toHex(publicKey3) +
          num2bin(balance0, DataLen) +
          num2bin(balance1, DataLen),
      ].join(" ");
      tx.addOutput(
        new bsv.Transaction.Output({
          script: bsv.Script.fromASM(newLockingScript0),
      ...
      const sig = signTx(
        tx,
        inputIndex == 0 ? privateKey1 : privateKey2,
        inputIndex == 0 ? lockingScript0 : lockingScript1,
        inputSatoshis,
        inputIndex
      );
    ...
    result = testMerge(0, expectedBalance0, expectedBalance1 + 1).verify({
      inputIndex: 0,
    });
    expect(result.success, result.error).to.be.true;
    result = testMerge(0, expectedBalance0 - 1, expectedBalance1).verify({
      inputIndex: 0,
    });
    expect(result.success, result.error).to.be.false;

    // input1 only checks balance1
    result = testMerge(1, expectedBalance0 - 1, expectedBalance1).verify({
      inputIndex: 1,
    });
    expect(result.success, result.error).to.be.true;
    result = testMerge(1, expectedBalance0, expectedBalance1 + 1).verify({
      inputIndex: 1,
    });
    expect(result.success, result.error).to.be.false;

    // both balance0 and balance1 have to be right to pass both checks of input0 and input1
    result = testMerge(0, expectedBalance0, expectedBalance1).verify({
      inputIndex: 0,
    });
    expect(result.success, result.error).to.be.true;
    result = testMerge(1, expectedBalance0, expectedBalance1).verify({
      inputIndex: 1,
    });
    expect(result.success, result.error).to.be.true;
  });
  ...
  it("should succeed when one token UTXO is burnt", () => {
    ...
    result = testBurn(privateKey1).verify({
      tx: tx,
      inputIndex,
      inputSatoshis,
    });
    ...
    // unauthorized key
    result = testBurn(privateKey2).verify({
      tx: tx,
      inputIndex,
      inputSatoshis,
    });
